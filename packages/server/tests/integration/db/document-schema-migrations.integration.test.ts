...
function getDatabaseUrl(): URL {
  ...
  const rawUrl =
    process.env.DOCUMENT_SCHEMA_MIGRATIONS_REAL_DATABASE_URL ??
    envFromFile.TEST_DATABASE_URL ??
    envFromFile.DATABASE_URL;

  if (!rawUrl) {
    throw new Error(
      'Real document schema migration integration test requires DOCUMENT_SCHEMA_MIGRATIONS_REAL_DATABASE_URL or packages/server/.env.development.local'
    );
  }
