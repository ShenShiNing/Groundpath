...
// Backend error codes are runtime values, so this utility keeps the dynamic-key bridge local.
function translateDynamicKey(key: string): string {
  return (i18n.t as unknown as DynamicTranslate)(key);
}
...
export function translateApiError(
  ...
  if (code) {
    ...
    if (i18n.exists(key)) {
      return translateDynamicKey(key);
    }
  }
