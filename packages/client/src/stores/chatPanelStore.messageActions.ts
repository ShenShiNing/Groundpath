...
import { invalidateConversationQueries, getChatErrorMessage } from './chatPanelStore.helpers';

type Set = (
  partial: Partial<ChatPanelState> | ((state: ChatPanelState) => Partial<ChatPanelState>)
) => void;
type Get = () => ChatPanelState;

export function createSendMessageAction(set: Set, get: Get): ChatPanelState['sendMessage'] {
  return async (
    content: string,
...
export function createEditMessageAction(set: Set, get: Get): ChatPanelState['editMessage'] {
  return async (
    messageId: string,
    content: string,
    getAccessToken: () => string | null,
...
export function createRetryMessageAction(set: Set, get: Get): ChatPanelState['retryMessage'] {
  return async (
    messageId: string,
    getAccessToken: () => string | null,
    stream?: StreamControls
...
export function createStopGenerationAction(set: Set, get: Get): ChatPanelState['stopGeneration'] {
  return () => {
