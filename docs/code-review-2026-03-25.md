...
| #        | 问题                              | 文件                        | 描述                                                               |
| -------- | --------------------------------- | --------------------------- | ------------------------------------------------------------------ |
| ~~M-9~~  | ~~可选认证中会话失效处理~~ ✅     | `auth.middleware.ts:85-114` | 令牌有效但会话失效时返回 401，并复用主认证校验逻辑                 |
| ~~M-10~~ | ~~刷新令牌重放攻击检测不完整~~ ✅ | `token.service.ts:42-85`    | 刷新令牌重放/复用时统一撤销所有会话并使 access token 立即失效      |
| ~~M-11~~ | ~~资源所有权验证缺乏统一模式~~ ✅ | 各服务实现                  | 新增集中式资源所有权中间件，并通过模块 `public/ownership` 出口复用 |
...
| ~~M-19~~ | ~~日志大表缺少分区策略~~ ✅       | messages, login_logs, operation_logs   | `login_logs` / `operation_logs` 已按 `created_at` 月分区并自动补未来分区；`messages` 因外键限制改为软删会话定期物理清理 |
...
| 中     | 生产环境真实来源 IP 透传       | 多层代理下不应记录 Docker 私网地址，否则国家/城市/时区/ISP 会全部为空    |
