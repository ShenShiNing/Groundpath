...
| 优先级    | 必做事项                                      | 原因                                                                   |
| --------- | --------------------------------------------- | ---------------------------------------------------------------------- |
| **Now-1** | 完善 backfill 调度、进度与批次治理            | backfill 首版已落地，但还缺进度统计与运维能力                          |
| **Now-2** | 补专门的 integration / e2e / client UI 测试   | 目前测试以 unit / service / error-injection 为主，少量专项集成测试仍缺 |
| **Now-3** | 做缓存收益验证与进一步 selective invalidation | 缓存已进第二阶段，但仍缺命中率 / token 节省的量化验证                  |
...
1. **PDF 技术验证已收口**
   - 选型结论：统一采用 `docling` 作为唯一的结构化 RAG PDF 运行时（3/3 成功，avg 13.3s）
   - `marker` 运行时代码及 `pdf-parse` 结构化路径已移除；`pdf-parse` 仅保留在 `document-storage.service.ts` 中的轻量文本提取
2. **backfill 仅完成首版**
...
6. **专项测试矩阵仍未完全补齐**
   - 已有较多 unit / service / error-injection 测试，且已新增 `docling` parser fixture、`pdf runtime`、`outline_search / node_read / ref_follow` integration 覆盖
...
| **PDF**          | `docling`（唯一结构化运行时）               | 见 §5.2                        |
| **TXT / 纯文本** | 启发式分段（空行 + 行首模式匹配）           | 无法识别结构时整文档作为单节点 |

### 5.2 PDF 结构解析策略

PDF 是结构解析难度最高的格式。经过 P-1 技术验证，统一采用 docling：

1. **唯一运行时：docling**
   - 对学术论文、双栏排版、书籍均表现良好
   - 3/3 样本成功，avg 13.3s
   - 输出 Markdown 可直接复用 Markdown 结构化解析逻辑
   - marker 经验证不稳定（2/3 失败），pdf-parse 仅产生纯文本无法生成结构化节点树，均已移除
2. **兜底：LLM 辅助结构推断**
   - 仅在解析置信度不足时触发
...
2. `parserRuntime`：`docling / llm-assisted / fallback`
3. `parseConfidence`
4. `headingCount`
5. `orphanNodeRatio`
6. `pageCoverage`
...
2. 已完成 `docling` quick 样本验证、真实 runtime 接线、normalize v1（含 front matter / 目录点线 / 公式占位 / 锚点归一化）与 parser/search/read/follow integration 覆盖。
3. PDF 运行时选型已收口：统一 docling，marker/pdf-parse 结构化运行时代码已移除。

当前剩余动作：

1. 补一次完整的 token / 延迟 / citation 样例记录。
...
1. Worker 已支持长短文档路由、Markdown / DOCX / PDF 首版结构解析、解析失败回退 chunk；`docling` 真实 runtime 已接入 `pdfStructureParser.parseFromStorage()`。
...
3. `front matter` 显式标注、`table / figure / appendix` 子节点与更细粒度 citation excerpt 已落地到 parser/search/read/follow 链路。
...
4. 建立旧文档批量回填任务。
5. 建立离线评测与线上观测（成功率、延迟、工具调用次数、预算耗尽率）。
6. 优化 Worker 并发、重试、死信、缓存。
7. 对高频节点和目录摘要做缓存。
